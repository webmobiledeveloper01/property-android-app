...
function Kitchen({ title, id,item,cancelKitchen }) {
    ...
    const onOpenCeling = useCallback(() => {
        //invisible previous dropdown

    }, []);
    const onOpenBench = useCallback(() => {
        //invisible previous dropdown

    }, []);
    const onOpenTaps = useCallback(() => {
        //invisible previous dropdown

    }, []);
    const onOpenCup = useCallback(() => {
        //invisible previous dropdown

    }, []);
    const onOpenRange = useCallback(() => {
        //invisible previous dropdown
    ...
    return (
        <View style={{ marginVertical: 20, backgroundColor: colors.borderColor, padding: 20, }}>
         ...
         <Image style={{height:20,width:20}} source={require('../../assets/cross.png')} />
                ...
                <Text style={[styles.headerText, { marginVertical: 5 }]}>Kitchen {item?.id}</Text>
                </Column>
                </Row>
            {renderFloorItem(title = 'Floor', id = '1')}
            {renderWallItem(title = 'Wall', id = '2')}
            {renderDoorItem(title = 'Doors and windows', id = '3')}
            {renderCeilingItem(title = 'Ceiling', id = '4')}
            {renderBenchtopItem(title = 'Benchtop', id = '5')}
            {renderTapsItem(title = 'Taps', id = '6')}
            {renderCupboardsItem(titi = 'Cupboards', id = '7')}
            {renderRangehoodItem(title = 'Range hood', id = '8')}
            {renderPictures()}
        </View>
    );
}
