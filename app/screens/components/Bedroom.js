...
function Bedroom({ title, id,item, cancelBedroom }) {
    ...
    return (
        <View style={{ marginVertical: 20, backgroundColor: colors.borderColor, padding: 20, }}>
            ...
            {renderFloorItem(title = 'Wall', id = '1')}
            {renderWallItem(title = 'Floor', id = '2')}
            {renderDoorItem(title = 'Windows', id = '3')}
            {renderCeilingItem(title = 'Doors', id = '4')}
            {renderBenchtopItem(title = 'Ceiling', id = '5')}
            {renderTapsItem(title = 'Built in wardrobes', id = '6')}
            {renderPictures()}
        </View>
    );
}
